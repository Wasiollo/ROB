function [votes, classifiers_votes] = voting(tset, clsmx)
...
% votes - output matrix of votes cast by all one-versus-one classifiers
	...
	for i=1:size(clsmx,1)
		...
		classifiers_votes{1, i} = clsmx(i, 1:2);
		classifiers_votes{2,i} = zeros(size(votes));
		...
		votes(res >= 0, pid) += 1;
    classifiers_votes{2, i}(res >= 0, pid) +=1;
		...
		votes(res < 0, nid) += 1;
    classifiers_votes{2,i}(res < 0, nid) += 1;
	end
