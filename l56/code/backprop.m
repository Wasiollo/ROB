function [outputNetwork terr] = backprop(tset, tslb, layersWeights, lr)
...
% terr - total squared error of the ANN
	...
	for i = layersNumber - 1 : -1 : 1
		d{i} = (d{i+1} * layersWeights{i}') .* [actdf(act{i}) ones(setsNumber, 1)];
		d{i} = d{i}(:, 1:end - 1);
		D{i} = d{i + 1}' * [act{i} ones(setsNumber, 1)];
		layersGradient{i} = lr * D{i}';
	endfor

	terr = 0.5 * sum((act{layersNumber} - prefferedOutput)(:).^2) / setsNumber;
